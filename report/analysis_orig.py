...
def performanceOverTime(df, head_fixation_date=None, single_session=None,
  ...
  for date_sessionnum, block in sessions:
    ...
    block.ReactionTime = block.ReactionTime.astype(np.float32)
    valid_reaction_time = block.ReactionTime[block.ReactionTime != -1]
    print("valid_reaction_time:", valid_reaction_time.dtype)
    reaction_time.append(valid_reaction_time.mean())
    reaction_time_std.append(valid_reaction_time.std())
    used_feedback_delay.append(block.FeedbackTime.mean())
...
def _psych(df, PsycStim_axes, color, linewidth, legend_name, plot_points=True,
    ...
    #ndxNan = df.ChoiceLeft.isnull()
    #ndxChoice = df.ForcedLEDTrial == 0
    df_choice_made = df[df.ChoiceLeft.notnull()]
    df_no_cue = df_choice_made[df_choice_made.ForcedLEDTrial == 0]
    #df_light_cued = df_choice_made[df_choice_made.ForcedLEDTrial == 1]
    #StimDV = df_choice_made.DV
    StimDV = df_no_cue.DV # TODO: Loop and create for light-cued as well
    if plot_points:
      StimBin = 10
      EXTRA_BIN=2
      ...
      # Choice trials
      #PsycY = df_no_cue.ChoiceLeft.groupby(BinIdx[~ndxNan & ndxChoice]).mean()
      PsycY = df_no_cue.ChoiceLeft.groupby(BinIdx).mean()
      PsycY *= 100 # Convert to percentile
      PsycX = (((np.unique(BinIdx)+1)/StimBin)*2)-1-(EXTRA_BIN*(1/StimBin))
    ...
    if len(df_no_cue) > 1:
        ...
        if GLM:
          ...
          try:
            ...
            print("skipping GLM fit for for session(s):",
                  df_no_cue.Date.unique())
            print("PsycX len: ", len(x), len(y))
            return None, None
          #print(glm_res.summary())
          intercept, slope = glm_res.params
          ...
          try:
            ...
            print("skipping sigmoidal fit for for session(s):",
                  df_no_cue.Date.unique())
            print("PsycX len: ", len(x), len(y))
            return None, None
          else:
        ...
        if len(legend_name):
          legend_name="{}{}".format(legend_name,
                    "" if not plot_points else " ({:,} trials)".format(len(df_no_cue)))
        else:
          legend_name=None
        y_points *= 100 # Convert to percentile
        ...
        PsycStim_axes.plot(x_sampled, y_points, color=color,
