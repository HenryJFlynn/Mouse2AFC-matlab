from analysis import splitByDV
...
def metricByOpto(df, *, trim_start, trim_end, min_count_per_animal_group,
  ...
  res_dict = {"Name":[], "Metric":[], "BrainRegion": [], "StartState":[], "DV":[]}
  ...
  for (brain_region, start_state), region_df in df.groupby(
    ...
    for dv_range, single_dv, dv_df  in splitByDV(region_df, combine_sides=True):
      _optoCalcMetricsRates(res_dict=res_dict, brain_region=brain_region, start_state=start_state,
                              single_dv=single_dv, animal_name="All Animals", animal_df=dv_df,
                              df_cols=df_cols, min_count_per_animal_group=min_count_per_animal_group)
      for animal_name, animal_df in dv_df.groupby(dv_df.Name):
        _optoCalcMetricsRates(res_dict=res_dict, brain_region=brain_region, start_state=start_state,
                              single_dv=single_dv, animal_name=animal_name, animal_df=animal_df,
                              df_cols=df_cols, min_count_per_animal_group=min_count_per_animal_group)
  ...
  for metric, metric_df in res_df.groupby(res_df.Metric):
    ...
    optoPlotOptoMetricRaw(col_name=metric, df=metric_df, save_figs=save_figs, save_prefix=save_prefix)

def _optoCalcMetricsRates(*, res_dict, brain_region, start_state, single_dv, animal_name,
    animal_df, df_cols, min_count_per_animal_group):
    def calcRate(df, df_col, is_binary, condition, dict_prefix):
        ...
        if is_binary:
          ...
          for cat, cat_df in [("All", col), ("Correct", col_correct),
            ...
            for stat_name, stat_val in [("Mean", cat_df.mean()), ("SEM", cat_df.sem()),
                                        ("Count", len(cat_df))]:
              key = f"{key_start}{stat_name}"
              if key not in ret_dict:
                raise ValueError(f"Wrong key: {key}. Keys are: {ret_dict.key()}")
    ...
    for col_name, is_binary, filter_cond in df_cols:
      ...
      control_res = calcRate(control_trials, col_name, is_binary, filter_cond, "Control")
...
def optoPlotOptoMetricRaw(*, col_name, df, save_figs, save_prefix):
  for (brain_region, start_state), region_df in  df.groupby(
    ...
    for animal_name, animal_df in region_df.groupby(region_df.Name):
      #tmplt = {"ControlCorrect": [], "ControlIncorrect": [], "OptoCorrect":[], "OptoIncorrect":[]}
      Xs = {"ControlCorrect": [], "ControlIncorrect": [], "OptoCorrect":[], "OptoIncorrect":[]}
      Ys = {"ControlCorrect": [], "ControlIncorrect": [], "OptoCorrect":[], "OptoIncorrect":[]}
      YSem = {"ControlCorrect": [], "ControlIncorrect": [], "OptoCorrect":[], "OptoIncorrect":[]}
      annotations = {"ControlCorrect": [], "ControlIncorrect": [], "OptoCorrect":[], "OptoIncorrect":[]}
      def checkSingleVal(series, key):
        if len(series) != 1:
          raise RuntimeError(f"Found {len(series)} for key: {key} in {animal_name}")
      ...
      for dv, dv_df in animal_df.groupby(animal_df.DV):
        for metric_name in ("Correct", "Incorrect"):
          for prefix in ("Control", "Opto"):
            ...
            pts_total = str(int(checkSingleVal(dv_df[f"{key}Count"], key)))
            annotations[key].append(pts_total)
      #if len(Xs) < 3:
      #    return
      ax = plt.axes()
      ax.set_title(f"{animal_name} {col_name} - {brain_region}")
      for cat_name, cat_color in [("Correct", 'g'), ("Incorrect", 'r')]:
        for group_name, linestyle, marker in [("Control", '-', '.'), ("Opto", '--', 'x')]:
          ...
          xs = Xs[key]
          ys = np.array(Ys[key])
          ax.plot(xs, ys, label=key, color=cat_color, linestyle=linestyle, marker=marker)
          y_sem_upper = ys + YSem[key]
          y_sem_lower = ys - YSem[key]
          # print("Y sem upper:", y_sem_upper)
          # print("Y sem lower:", y_sem_lower)
          ax.fill_between(xs, y_sem_upper, y_sem_lower, alpha=0.2, color=cat_color)
          for x, y, _str in zip(xs, ys, annotations[key]):
            ax.annotate(_str, (x+0.02, y), fontsize=10)
      ax.legend(handlelength=3)
      if save_figs:
        descr = col_name[4:] if col_name.startswith("calc") else col_name
        analysis.savePlot(save_prefix + f"{animal_name}_{descr}_raw_{brain_region}_{start_state}")
      plt.show()
...
def _optoPlotOptoMetricAnimal(animal_name, df, ax, annotate, **plot_kwargs):
    ...
    for dv, dv_df in df.groupby(df.DV):
      pts_total = f"Control: {dv_df.ControlCount.sum():,} - Opto: {dv_df.OptoCount.sum():,}"
      Xs.append(dv)
