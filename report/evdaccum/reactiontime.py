...
from report.splitdata import _splitByBins
from .evdutils import plotSides, plotShortLongWT, fltrSsns, timeHist
...
def _loopDVBins(df, dv_bins_edges, animal_name, **kargs):
  if len(dv_bins_edges):
    ...
    for dv_rng, _, dv_df in _splitByBins(df, dv_bins_edges, combine_sides=True):
...
def _reactionTimePerDF(df, *, animal_name, plots, periods, quantile_top_bottom,
  ...
  if plots & (Plots.MinSampleDistHist | Plots.ReactionTimeDistHist):
    ...
    if plots & Plots.ReactionTimeDistHist:
      ...
      timeHist(ax=axs[-1], df=df_accepted, col_name="ST", normalized=False,
              friendly_col_name="Reaction Time (s)", overstay_col=overstay_col,
              gauss_fit=False, max_x_lim=10, bins_per_sec=4,
              plot_only_all=False, stacked=True, quantiles_to_plot=None,
  ...
  if plots & (Plots.ReactionTimeVsDiff | Plots.ReactionTimeVsDiffHist):
    plotSides(df_accepted, col_name="ST", friendly_col_name="Reaction Time",
